...
function AdminSubmitPrayerRequestPage({ user, onBack }) {
  const { t } = useLanguage();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addDoc(collection(db, 'prayer_requests'), {
        personName,
        description,
        submittedBy: user?.name || user?.email || 'Admin',
        leaderId: user?.id || 'admin',
    ...
    <Box>
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          <TextField 
            name="personName" 
            label={t('prayer.personName') || 'Person Name'} 
            fullWidth 
            required 
          ...
          <TextField 
            name="description" 
            label={t('prayer.desc') || 'Description'} 
            fullWidth 
            required 
            multiline
            rows={4}
          ...
          >
            {submitLoading ? 'Submitting...' : (t('gen.submit') || 'Submit')}
          </Button>
        </form>
      </Paper>
    </Box>
...
export default AdminSubmitPrayerRequestPage;
